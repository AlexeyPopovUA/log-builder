...
const exportLibraryName = "logBuilder";
const cjsLibraryName = "log-builder";
const projectName = "log-builder";
const getPostfix = mode => mode === "production" ? ".min" : "";
const getFolderPostfix = mode => mode === "development" === true ? "debug" : "production";
...
const getBrowserConf = (buildMode, target) => ({
    buildMode,
    outputFile: `${projectName}${getPostfix(buildMode)}.js`,
...
/*const getNodeConf = (buildMode, target) => ({
    buildMode,
    outputFile: `${projectName}.node${getPostfix(buildMode)}.js`,
    outputFolder: `${getTargetFolder(target)}${getFolderPostfix(buildMode)}`,
    target: "node",
...
module.exports = env => {
    ...
    for (const mode of modes) {
        ...
        for (const target of targets) {
            ...
            buildConfigs.push(getConfiguration(/*isBrowser(target)*/true, config, exportLibraryName, watch));
        }
    }
